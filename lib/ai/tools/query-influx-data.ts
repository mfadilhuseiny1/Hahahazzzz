...
let clientInstance: InfluxDB | null = null;
...
export const queryInfluxData = tool({
  ...
  execute: async ({ query, timeRange, parameters }) => {
    ...
    // Reset client jika belum ada
    if (!clientInstance) {
      clientInstance = new InfluxDB({
        url: process.env.INFLUXDB_URL,
        token: process.env.INFLUXDB_TOKEN,
        timeout: 10000
      });
    ...
    try {
      const queryApi = clientInstance.getQueryApi(process.env.INFLUXDB_ORG);
      ...
      return new Promise((resolve, reject) => {
        const timeout = setTimeout(() => {
          clientInstance = null; // Reset on timeout
          reject({ error: 'Query timeout', message: 'InfluxDB timeout' });
        }, 8000);
        ...
        queryApi.queryRows(fluxQuery, {
          ...
          error(error) {
            ...
            clientInstance = null; // Reset on error
            reject({
              error: 'Failed to retrieve sensor data',
              message: error.message
            });
      ...
      clientInstance = null; // Reset on connection error
      return {
        error: 'Failed to retrieve sensor data',
        message: error instanceof Error ? error.message : 'Unknown error'
      };
